from SF_TRON_Ext.utils.PPO.Buffer import *
...
class Actor_Critic:
    def __init__(self, PPO_Config, Env_Config, index=0):
        ...
        """
        self.index = index
        self.model_suffix = "_est" if index == 1 and PPO_Config.EstimatorParam.enabled else ""
        # PPO parameter
        ...
        # state parameter
        if index == 1 and PPO_Config.EstimatorParam.enabled:
            self.state_dim = PPO_Config.CriticParam.residual_state_dim
        else:
            self.state_dim = PPO_Config.CriticParam.base_state_dim
    ...
    def update(self, previous_critic=None):
        ...
        # 加上这段代码，它会自动在项目根目录生成一个 log.csv
        with open("training_log.csv", "a") as f:
            import datetime
            now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            f.write(f"{now}, {reward_sum}, {critic_loss.item()}, {actor_loss.item()}\n")

    def save_best_model(self):
        torch.save(self.actor.state_dict(), f'model/NN_Model/actor{self.index}{self.model_suffix}.pth')
        torch.save(self.critic.state_dict(), f'model/NN_Model/critic{self.index}{self.model_suffix}.pth')

    def save_each_epi_model(self):
        torch.save(self.actor.state_dict(), f'model/NN_Model/actor{self.index}{self.model_suffix}_f.pth')
        torch.save(self.critic.state_dict(), f'model/NN_Model/critic{self.index}{self.model_suffix}_f.pth')

    def load_best_model(self):
        self.actor.load_state_dict(torch.load(f'model/NN_Model/actor{self.index}{self.model_suffix}.pth'))
        self.critic.load_state_dict(torch.load(f'model/NN_Model/critic{self.index}{self.model_suffix}.pth'))

    def load_each_epi_model(self):
        self.actor.load_state_dict(torch.load(f'model/NN_Model/actor{self.index}{self.model_suffix}_f.pth'))
        self.critic.load_state_dict(torch.load(f'model/NN_Model/critic{self.index}{self.model_suffix}_f.pth'))

    def save_checkpoint(self, episode):
        torch.save(self.actor.state_dict(), f'model/NN_Model/actor{self.index}{self.model_suffix}_ckpt{episode}.pth')
        torch.save(self.critic.state_dict(), f'model/NN_Model/critic{self.index}{self.model_suffix}_ckpt{episode}.pth')

...
def EnvSetup(file_path, dt, sub_step, agents_num, device, DomainRandomizationCfg, terrain_num_rows=15, terrain_num_cols=15):
    ...
    sub_terrains[f"stepping_stone"] = HfSteppingStonesTerrainCfg(
        ...
        stone_height_max=0,
        stone_width_range=(0.4, 0.8), # 0.5的宽度实际上是0.4m
    ...
    print(f"Environment initialization: num_row of terrains: {terrain_num_rows}x{terrain_num_cols}")

    """生成地形配置文件"""
    gen_cfg = TerrainGeneratorCfg(
        num_rows=terrain_num_rows,  # 太多了会出问题，比如碰撞体失效

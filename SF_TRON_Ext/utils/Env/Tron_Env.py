...
from SF_TRON_Ext.utils.Useful_Function.Useful_Function import *
...
class Tron_Env:
    def __init__(self, Env_Config, Robot_Config, PPO_Config):
        ...
        self.terrain_num_rows = Env_Config.EnvParam.terrain_num_rows
        self.terrain_num_cols = Env_Config.EnvParam.terrain_num_cols
        ...
        self.history_obs_buffer = torch.zeros(
            (self.agents_num, self.EstimatorParam.history_len, self.EstimatorParam.obs_dim),
            device=self.device,
        )
        self.external_force_label = torch.zeros((self.agents_num, self.EstimatorParam.latent_dim), device=self.device)
        self.push_time_remaining = torch.zeros((self.agents_num, 1), device=self.device)
        self.push_force_xy = torch.zeros((self.agents_num, 2), device=self.device)
        ...
        self.sim, self.scene = EnvSetup(
            ...
            self.terrain_num_rows,
            self.terrain_num_cols,
        )
    ...
    def prim_initialization(self, agent_index=None, reset_all=False):
        ...
        self.history_obs_buffer[agent_index] = 0
        self.external_force_label[agent_index] = 0
        self.push_time_remaining[agent_index] = 0
        self.push_force_xy[agent_index] = 0
    ...
    def update_estimator_history(self, state):
        """Store the non-visual observation history used by the disturbance estimator."""
        ...
        self.history_obs_buffer = torch.roll(self.history_obs_buffer, shifts=-1, dims=1)
        self.history_obs_buffer[:, -1, :] = estimator_obs
        return self.history_obs_buffer

    def get_estimator_history(self):
        return self.history_obs_buffer

    def augment_state_with_estimate(self, state, estimate):
        return torch.concatenate((state, estimate), dim=1)

    def _update_external_force_label(self):
        self.external_force_label[:] = 0
        if not self.DisturbanceCfg.enable_push:
            return

        self.push_time_remaining = torch.clamp(self.push_time_remaining - self.dt, min=0.0)
        push_probability = self.dt / max(self.DisturbanceCfg.push_interval, self.dt)
        start_push = (self.push_time_remaining <= 0) & (
            torch.rand((self.agents_num, 1), device=self.device) < push_probability
        )
        # Generate force direction/magnitude once at push start, reuse for entire push duration
        self.push_force_xy[start_push.flatten()] = self.DisturbanceCfg.max_force * (
            2 * torch.rand((int(start_push.sum()), 2), device=self.device) - 1
        )
        self.push_time_remaining[start_push] = self.DisturbanceCfg.push_duration

        active_push = self.push_time_remaining > 0
        self.external_force_label[:, 0:2] = self.push_force_xy * active_push.float()

    def _apply_external_force(self):
        if not self.DisturbanceCfg.enable_push:
            return
        forces = self.external_force_label.view(self.agents_num, 1, self.EstimatorParam.latent_dim)
        torques = torch.zeros_like(forces)
        self.scene["robot"].set_external_force_and_torque(
            forces=forces,
            torques=torques,
            body_ids=[self.DisturbanceCfg.body_id],
            is_global=True,
        )
    ...
    def get_current_observations(self):
        ...
        current_state = torch.concatenate(
            (self.joint_pos + current_noise[0],
             self.joint_vel + current_noise[1],
             self.body_ori + current_noise[2],
             self.angular_velocities + current_noise[3],
             self.sine_clock,
             self.cosine_clock,
             self.action,
             self.vel_cmd,
    ...
    def get_next_observations(self):
        ...
        next_state = torch.concatenate((self.next_joint_pos + next_noise[0],
                                        self.next_joint_vel + next_noise[1],
                                        self.next_body_ori + +next_noise[2],
                                        self.next_angular_velocities + +next_noise[3],
                                        self.sine_clock,
                                        self.cosine_clock,
                                        self.action,
                                        self.vel_cmd,
    ...
    def update_world(self, action):
        ...
        self.action = action
        self._update_external_force_label()
    ...
    def vel_tracking_reward(self):
        ...
        reward_vel_forward = - 1 * torch.abs(vel_forward - 1 * if_forward) + 1
